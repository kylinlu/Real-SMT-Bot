...
def run_bot(r, comments_replied_to):
	for comment in r.subreddit("test").comments(limit = None):
		if "persona" in comment.body.lower().split() and comment.id not in comments_replied_to and comment.author != r.user.me() and comment.subreddit != "megaten" and "bot" not in comment.author.name.lower():
